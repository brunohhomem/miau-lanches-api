...
async function main() {
  ...
  // Recuperar os ingredientes criados
  const allIngredients = await prisma.ingrediente.findMany();

  // Criar Lanches e associar Ingredientes
  const lanches = await Promise.all([
    prisma.lanche.create({
      data: {
        ...
        preco: 12.0,
        ingredientes: {
          connect: allIngredients.filter((i) =>
            ['Queijo', 'Hambúrguer', 'Tomate', 'Alface'].includes(i.descricao),
          ),
        },
      },
    }),
    prisma.lanche.create({
      data: {
        ...
        ingredientes: {
          connect: allIngredients.filter((i) =>
            ['Queijo', 'Hambúrguer', 'Bacon', 'Molho Especial'].includes(
              i.descricao,
            ),
          ),
        },
    ...
    prisma.lanche.create({
      data: {
        ...
        ingredientes: {
          connect: allIngredients.filter((i) =>
            ['Queijo', 'Tomate', 'Alface', 'Molho Especial', 'Picles'].includes(
              i.descricao,
            ),
          ),
        },
    ...
    prisma.lanche.create({
      data: {
        ...
        ingredientes: {
          connect: allIngredients.filter((i) =>
            ['Queijo', 'Tomate', 'Alface', 'Molho Especial', 'Ovo'].includes(
              i.descricao,
            ),
          ),
        },
    ...
    prisma.lanche.create({
      data: {
        ...
        ingredientes: {
          connect: allIngredients.filter((i) =>
            ['Queijo', 'Hambúrguer', 'Bacon', 'Cebola Caramelizada'].includes(
              i.descricao,
            ),
          ),
        },
